...
class Common_modal extends CI_Model {
    ...
    function getAllOrderDesc($table,$order){
        $this->db->select('*');
		$this->db->from($table);
    ...
    function getrandomOne($table,$field){
        $this->db->select('*'); 
        $this->db->from($table);
        // $this->db->where($idName, $val);
    ...
    function checkValue($id,$name){
		...
		$this->db->from('inventory');
	   	$this->db->where('inv_name',$name);
	   	$this->db->where_not_in('inv_id',$id);
	   	$this->db->limit(1);
		$query = $this->db->get();
		if($query->num_rows() == 1){
			return true;
    ...
    function get_invoice($id){
        $this->db->select('s.*,so.*,i.inv_name');
        $this->db->from('sales s');
        $this->db->where('s.sale_id', $id);
        $this->db->join('sales_info so', 'so.sale_id=s.sale_id');
        $this->db->join('inventory i', 'i.inv_id=so.inv_id');
        $query = $this->db->get();
        return $query->result();        
    }
}
